...
describe('Line editor', () => {
    it('should add anchor to line editor', () => {
        const line = new Konva.Line({points: [10, 10, 30, 30]})
    ...
    it('should update control position when line editor', () => {
        const line = new Konva.Line({points: [10, 10, 30, 30]})
        const editor = new LineEditor()
        editor.attach(line)
    ...
    it('should change line points when dragging anchor', () => {
        ...
        const anchor = editor.findOne('.1-anchor')
    ...
    it('should add new anchor on editor when dragging control point', () => {
        ...
        const control = editor.findOne('.1-control')

        expect(control.draggable()).toEqual(true)

        const stopDrag = vi.spyOn(control, 'stopDrag')
        const startDrag = vi.spyOn(editor.findOne('.1-anchor'), 'startDrag')
    ...
    it('should remove anchor when double click', function () {
        const line = new Konva.Line({points: [10, 10, 20, 20, 30, 30]})
        const editor = new LineEditor()
        editor.attach(line)
